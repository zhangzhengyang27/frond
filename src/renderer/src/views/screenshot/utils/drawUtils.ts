...
export function drawDragCircle(ctx: CanvasRenderingContext2D, x: number, y: number): void {
  ctx.lineWidth = 1
...
export function isHitCircle(canvas: HTMLCanvasElement | null, e: MouseEvent, point: Point): boolean {
  if (!canvas) {
    return false
  }
