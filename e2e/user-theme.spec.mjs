/**
 * Frond · E2E：用户主题文件端到端（#12 Phase 2）
 *
...
const getMainWindow = async () => {
  ...
  while (Date.now() < deadline) {
    for (const w of app.windows()) {
      try {
        // 按 URL 认，不按 title：胶囊窗的 title 也含 Frond，胶囊一开就会先匹配到它，
...
test('主题文件被列出 → 点选后 CSS 变量生效 → 切回内置后覆盖摘除', async () => {
  ...
  expect(await page.evaluate(() => document.getElementById('frond-user-theme-vars') === null)).toBe(
    true
  )
})
