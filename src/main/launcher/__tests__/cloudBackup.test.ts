...
describe('pickPruneCandidates', () => {
  const names = (ts: number[]): string[] => ts.map((t) => `leaf-full-${t}.db.enc`)
  ...
  it('非法文件名（无时间戳）视为最旧优先删除', () => {
    const prune = pickPruneCandidates(['leaf-full-x.db.enc', ...names([100, 200, 300])], 3)
    expect(prune).toEqual(['leaf-full-x.db.enc'])
  })

  it('tsFromFileName 解析', () => {
    expect(tsFromFileName('leaf-full-1726000000000.db.enc')).toBe(1726000000000)
